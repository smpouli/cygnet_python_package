...
path_to_installed_database= Path(__file__).parent/"cyg_data"/"cygnet.db"

if path_to_installed_database.exists():   
    # connexion to the database
    conn = sqlite3.connect(path_to_installed_database)
    Database = conn.cursor()
else:    
    download_url = "https://github.com/rowanhm/cygnet/releases/latest/download/cygnet.db.gz"
    
    gz_file = Path(__file__).parent/"cyg_data"/"cygnet.db.gz"
    ...
    try:
        ...
        with gzip.open(gz_file, "rb") as gz:
            with open(path_to_installed_database, "wb") as output:
        ...
        print ("An error occurred, please try again or look at the help file in the cyg_data folder.")
